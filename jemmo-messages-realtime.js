...
(() => {
  ...
  const VERSION = 'JEMMO LIVE V1 · MENSAJES TIEMPO REAL PRUEBA 01';
  ...
  function ensureNewConversationDialog() {
    ...
    backdrop.innerHTML = `
      <section class="jemmo-rt-new-sheet" role="dialog" aria-modal="true" aria-labelledby="jemmoRtNewTitle">
        ...
        <p>Busca a una persona por su correo de acceso o pega su UID de Firebase. Ambos usuarios deben haber iniciado sesión una vez con esta actualización.</p>
        <form class="jemmo-rt-new-row" id="jemmoRtSearchForm">
          <input id="jemmoRtUserSearch" type="text" inputmode="email" autocomplete="off" maxlength="160" placeholder="correo@ejemplo.com o UID">
          <button type="submit">BUSCAR</button>
        </form>
        <div class="jemmo-rt-search-result" id="jemmoRtSearchResult"><div class="jemmo-rt-help">Escribe el correo exacto de la persona.</div></div>
      </section>`;
  ...
  function openNewConversation() {
    ...
    if (result) result.innerHTML = '<div class="jemmo-rt-help">Escribe el correo exacto de la persona.</div>';
  }
  ...
  function peerData(conversation) {
    ...
    return {
      uid: peerUid,
      name: String(profile.name || fallback?.name || 'Usuario JEMMO'),
      email: String(profile.email || fallback?.email || ''),
      initial: initials(profile.name || fallback?.name || 'J'),
  ...
  function renderOnlinePeople(conversations) {
    ...
    if (headSpan) headSpan.textContent = 'Sin estados simulados';
    ...
    conversations.slice(0, 8).forEach(conversation => {
      ...
      button.type = 'button';
      button.innerHTML = `<span class="avatar-wrap"><span class="avatar" style="--a1:#6f2290;--a2:#190322">${esc(peer.initial)}</span></span><b>${esc(peer.name)}</b>`;
      button.addEventListener('click', () => openConversation(conversation.id));
      onlineList.appendChild(button);
    });

    if (!conversations.length) {
      onlineList.innerHTML = '<div class="jemmo-rt-help" style="width:100%">Pulsa ＋ para iniciar tu primera conversación real.</div>';
    }
  }
  ...
  function conversationRow(conversation) {
    ...
    button.innerHTML = `
      <span class="row-avatar round" style="--a1:#75239a;--a2:#190322;--accent:#a943ff">${esc(peer.initial)}</span>
  ...
  function renderMessages(messageDocuments) {
    ...
    if (!area) return;
    area.innerHTML = '<div class="day">MENSAJES CIFRADOS EN TRÁNSITO POR HTTPS · PRUEBA</div>';
  ...
  async function openConversation(conversationId) {
    ...
    if ($('chatAvatar')) $('chatAvatar').textContent = peer.initial;
    renderMessages([]);
  ...
  async function searchUser() {
    ...
    if (!result) return;
    if (!raw) {
      result.innerHTML = '<div class="jemmo-rt-help error">Escribe un correo o UID.</div>';
      return;
    }
    ...
    result.innerHTML = '<div class="jemmo-rt-help">Buscando usuario…</div>';
    const {
    ...
    try {
      ...
      if (!raw.includes('@')) {
        const byUid = await getDoc(doc(state.db, 'directorioMensajes', raw));
        if (byUid.exists()) targetSnapshot = byUid;
      } else {
        const lower = raw.toLocaleLowerCase('es');
        let matches = await getDocs(query(
          collection(state.db, 'directorioMensajes'),
          where('emailLower', '==', lower),
          limit(2)
        ));
        if (matches.empty) {
          matches = await getDocs(query(
            collection(state.db, 'directorioMensajes'),
            where('email', '==', raw),
            limit(2)
          ));
        }
        targetSnapshot = matches.docs[0] || null;
      }

      if (!targetSnapshot) {
        result.innerHTML = '<div class="jemmo-rt-help error">No aparece ese usuario. Debe iniciar sesión una vez con esta actualización para quedar disponible.</div>';
        return;
      }
      ...
      button.innerHTML = `<i>${esc(initials(target.name))}</i><span><b>${esc(target.name)}</b><small>${esc(target.email || target.uid)}</small></span>`;
  ...
  function normalizedUserProfile(uid, data) {
    ...
    const name = String(data.nombre || data.name || data.displayName || email.split('@')[0] || 'Usuario JEMMO').trim();
    return { uid, name, email };
  }

  async function ensureCurrentUserDocument() {
    const { doc, setDoc, serverTimestamp } = state.sdk.firestore;
    const profile = {
      uid: state.user.uid,
      name: displayNameFromUser(state.user),
      email: String(state.user.email || '').trim()
    };
    state.profile = profile;

    await setDoc(doc(state.db, 'directorioMensajes', state.user.uid), {
      email: profile.email,
