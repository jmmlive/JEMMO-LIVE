(()=>{
  'use strict';
  if(window.__JEMMO_PWA_REGISTER_43__)return;
  window.__JEMMO_PWA_REGISTER_43__=true;
  const RELEASE='pwa-real-task-live-room-43';
  ...
  function showInstallHelp(message){
    const existing=document.getElementById('jemmoInstallHelp43');existing?.remove();
    const el=document.createElement('div');el.id='jemmoInstallHelp43';el.style.cssText='position:fixed;z-index:50000;left:50%;bottom:calc(18px + env(safe-area-inset-bottom,0px));transform:translateX(-50%);width:min(92vw,440px);padding:12px 14px;border:1px solid #b946e8;border-radius:15px;background:#17021eee;color:#fff;font:700 12px/1.4 system-ui;box-shadow:0 12px 36px #000c;text-align:center';el.textContent=message;document.body.appendChild(el);setTimeout(()=>el.remove(),4200)
  }
  ...
  function loadHousePet(){
    ...
    if(!document.querySelector('link[data-jemmo-house-pet]')){const link=document.createElement('link');link.rel='stylesheet';link.href='./jemmo-house-pet.css?v=43';link.dataset.jemmoHousePet='43';document.head.appendChild(link)}
    if(!document.querySelector('script[data-jemmo-house-pet]')){const script=document.createElement('script');script.src='./jemmo-house-pet.js?v=43';script.defer=true;script.dataset.jemmoHousePet='43';script.onerror=()=>console.warn('JEMMO Mascota: no se pudo cargar el módulo.');document.head.appendChild(script)}
  }
