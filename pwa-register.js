(()=>{
  'use strict';
  if(window.__JEMMO_PWA_REGISTER_51__)return;
  window.__JEMMO_PWA_REGISTER_51__=true;
  const RELEASE='pwa-camera-vertical-51';
  ...
  function loadHousePet(){
    ...
    if(!document.querySelector('link[data-jemmo-house-pet]')){const link=document.createElement('link');link.rel='stylesheet';link.href='./jemmo-house-pet.css?v=51';link.dataset.jemmoHousePet='51';document.head.appendChild(link)}
    if(!document.querySelector('script[data-jemmo-house-pet]')){const script=document.createElement('script');script.src='./jemmo-house-pet.js?v=51';script.defer=true;script.dataset.jemmoHousePet='51';script.onerror=()=>console.warn('JEMMO Mascota: no se pudo cargar el módulo.');document.head.appendChild(script)}
  }
