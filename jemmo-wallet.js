...
(() => {
  ...
  function normalize(input) {
    ...
    const modern = Number(value.schemaVersion) >= 2 || ['jemmos','jems','crystals','jemsConfirmed','jemsPending'].some(key => Object.prototype.hasOwnProperty.call(value, key));
    let jemmos;
    let jems;
    let crystals;
    let jemsConfirmed;
    ...
    if (modern) {
      ...
      jemsPending = Math.max(0, Math.floor(Number(value.jemsPending) || 0));
      const hasFinanceSplit = Object.prototype.hasOwnProperty.call(value, 'jemsConfirmed') || Object.prototype.hasOwnProperty.call(value, 'jemsPending');
      jemsConfirmed = hasFinanceSplit
        ? Math.max(0, Math.floor(Number(value.jemsConfirmed) || 0))
        : Math.max(0, Math.floor(Number(value.jems ?? value.earnings) || 0));
      jems = jemsConfirmed + jemsPending;
      crystals = Number(value.crystals ?? value.diamonds) || 0;
    } else {
      jemmos = Number(value.coins) || 0;
      crystals = Number(value.diamonds) || 0;
      jemsConfirmed = Math.round((Number(value.earnings) || 0) * 100);
      jemsPending = 0;
      jems = jemsConfirmed;
    }
    ...
    const wallet = {
      ...
      jemsConfirmed,
      jemsPending,
      jemsWithdrawn: Math.max(0, Math.floor(Number(value.jemsWithdrawn) || 0)),
      crystals,
      coins: jemmos,
      diamonds: crystals,
      earnings: jems,
      ...
      ledger: Array.isArray(value.ledger) ? value.ledger.slice(0, 400) : [],
      lots: Array.isArray(value.lots) ? value.lots : [],
      pendingCredits: Array.isArray(value.pendingCredits) ? value.pendingCredits : [],
      earningsHistory: Array.isArray(value.earningsHistory) ? value.earningsHistory : [],
      withdrawals: Array.isArray(value.withdrawals) ? value.withdrawals : [],
  ...
  function consumeLots(wallet, amount) {
    if (!Array.isArray(wallet.lots) || !wallet.lots.length) return;
  ...
  function movement(type, title, detail, amount, tone = 'neutral') {
    ...
    wallet.history.unshift({
      ...
      type, title, detail, amount, tone, createdAt: Date.now()
    });
  ...
  function addCoins(amount, meta = {}) {
    ...
    wallet.lots.push({
      ...
      method: meta.method || 'wallet-global',
      label: meta.detail || 'Recarga desde Monedero global',
      risk: 'confirmed',
      initialJemmos: amount,
      remaining: amount,
      createdAt: Date.now()
    });
    wallet.history.unshift({
    ...
    pushLedger(wallet, meta.type || 'recharge', meta.title || 'Recarga de prueba', { amountJemmos: amount, method: meta.method || 'wallet-global', status: 'confirmed' });
  ...
  function spendCoins(amount, meta = {}) {
    ...
    pushLedger(wallet, meta.type || 'gift', meta.title || 'Regalo enviado', { amountJemmos: -amount, detail: meta.detail || '', status: 'confirmed' });
  ...
  function addJems(amount, meta = {}) {
    ...
    pushLedger(wallet, meta.type || 'gift-received', meta.title || 'Regalo recibido', { amountJems: amount, detail: meta.detail || '', status: 'confirmed' });
  ...
  function addCrystals(amount, meta = {}) {
    ...
    wallet.history.unshift({
      ...
      amountCrystals: amount,
      tone: 'positive',
      createdAt: Date.now()
    });
    pushLedger(wallet, meta.type || 'crystals', meta.title || 'Cristales añadidos', { amountCrystals: amount, detail: meta.detail || '' });
  ...
  function exchangeCalculation() {
    ...
    return { valid: wallet.jemsConfirmed >= requested, input: requested, output: requested * 10, type, message: `Recibirás <b>${formatNumber(requested * 10)} JEMMOS</b> usando ${formatNumber(requested)} JEMS confirmados.${wallet.jemsConfirmed < requested ? ' Saldo insuficiente.' : ''}` };
  }
  ...
  function walletVisible() {
    return Boolean(byId('jw-sheet') && !byId('jw-sheet').hidden);
  }
  ...
  function bindUi() {
    ...
    byId('jw-exchange-confirm')?.addEventListener('click', () => {
      ...
      if (result.type === 'jemmos-crystals') {
        wallet.jemmos -= result.input; wallet.coins = wallet.jemmos; consumeLots(wallet, result.input); wallet.crystals += result.output; wallet.diamonds = wallet.crystals;
        ...
        wallet.crystals -= result.input; wallet.diamonds = wallet.crystals; wallet.jemmos += result.output; wallet.coins = wallet.jemmos; wallet.lots.push({id:`lot_${Date.now()}`,method:'exchange',label:'Cambio desde CRISTALES',risk:'confirmed',initialJemmos:result.output,remaining:result.output,createdAt:Date.now()});
        ...
        wallet.jemsConfirmed -= result.input; wallet.jems = wallet.jemsConfirmed + wallet.jemsPending; wallet.earnings = wallet.jems; wallet.jemmos += result.output; wallet.coins = wallet.jemmos; wallet.lots.push({id:`lot_${Date.now()}`,method:'exchange',label:'Cambio desde JEMS',risk:'confirmed',initialJemmos:result.output,remaining:result.output,createdAt:Date.now()});
    ...
    byId('jw-withdraw-confirm')?.addEventListener('click', () => {
      ...
      if (amount > wallet.jemsConfirmed) return toast('No tienes JEMS confirmados suficientes.');
      if (!confirm(`¿Retirar ficticiamente ${formatNumber(amount)} JEMS?`)) return;
      wallet.jemsConfirmed -= amount; wallet.jemsWithdrawn += amount; wallet.jems = wallet.jemsConfirmed + wallet.jemsPending; wallet.earnings = wallet.jems;
