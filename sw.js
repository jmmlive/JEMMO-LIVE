/* JEMMO LIVE V1 · TARIFA AUDIO ROOM 800 Y SILENCIO MODERADO PRUEBA 37 */
const CACHE = 'jemmo-live-v1-task-rates-37-20260727';
const APP_SHELL = [
  './',
  './index.html',
  './acceso.html',
...
async function injectJemmoScripts(response, url) {
  ...
  if (UNREAD_PAGES.has(page)) {
    html = injectBeforeBody(
      html,
      '<script src="./jemmo-unread-badge.js" data-jemmo-injected="task-rates-37"></script>'
    );
  }
  if (page === '/mensajes.html' && !html.includes('jemmo-messages-realtime.js')) {
    html = injectBeforeBody(
      html,
      '<script src="./jemmo-messages-realtime.js" data-jemmo-injected="task-rates-37"></script>'
    );
  }
  if (['/yo.html','/mensajes.html','/chili-ia.html'].includes(page) && !html.includes('jemmo-social.js')) {
    html = injectBeforeBody(
      html,
      '<script type="module" src="./jemmo-social.js" data-jemmo-injected="task-rates-37"></script>'
    );
  }
  if (['/yo.html','/casa-demo.html'].includes(page) && !html.includes('jemmo-role-lab.js')) {
    html = injectBeforeBody(
      html,
      '<script type="module" src="./jemmo-role-lab.js" data-jemmo-injected="task-rates-37"></script>'
    );
  }
  if (page === '/yo.html') {
    html = injectBeforeBody(
      html,
      '<script type="module" src="./jemmo-cloud-profile.js" data-jemmo-injected="task-rates-37"></script>'
    );
  }
  ...
  headers.set('x-jemmo-version', 'task-rates-37');
