/* JEMMO LIVE V1 · ACCESO ESTABLE PRUEBA 04 */
const CACHE = 'jemmo-live-v1-acceso-estable-prueba-04-20260724';
const APP_SHELL = [
  './',
  './index.html',
  './acceso.html',
...
async function injectJemmoScripts(response, url) {
  ...
  if (UNREAD_PAGES.has(page)) {
    html = injectBeforeBody(
      html,
      '<script src="./jemmo-unread-badge.js" data-jemmo-injected="acceso-estable-prueba-04"></script>'
    );
  }
  if (page === '/mensajes.html') {
    html = injectBeforeBody(
      html,
      '<script src="./jemmo-messages-realtime.js" data-jemmo-injected="acceso-estable-prueba-04"></script>'
    );
  }
  ...
  headers.set('x-jemmo-version', 'acceso-estable-prueba-04');
