...
(() => {
  ...
  const DAY_MS=86400000,LEASE_MS=90000,MAX_FLUSH_SECONDS=120,LIVE_MAX_HOURS=3,AUDIO_ROOM_MAX_HOURS=2;
  ...
  async function flush(force=false){
    ...
    try{
      await s.runTransaction(s.db,async transaction=>{
        ...
        const targetHours=Math.max(1,Math.ceil(totalTargetMinutes/60));const payableHours=Math.min(Math.floor(liveSeconds/3600),LIVE_MAX_HOURS)+Math.min(Math.floor(roomSeconds/3600),AUDIO_ROOM_MAX_HOURS);const complete=payableHours>=targetHours;
  ...
  function sync(){if(targetRunning())void start();else void stop(inactiveReason())}
  ...
  window.JemmoHouseActivity={version:'44.0-test',getState:()=>({type:activityType,houseId,running,sessionId,sessionElapsedSeconds,cycleEndMs,leaseLost,reason:running?'active':inactiveReason(),seat:window.JemmoHouseRoomControls?.getState?.()?.localSeat||0}),flush:()=>flush(true),ensureTaskCycle};
